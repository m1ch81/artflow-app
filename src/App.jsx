...
const ArtFlowApp = () => {
  ...
  // Save anxiety level to Firestore
  const saveAnxietyLevel = async (level) => {
    if (user) {
  ...
  const saveSession = async (activity) => {
    if (user) {
      const newSession = {
        id: Date.now(),
      ...
      const userDocRef = doc(db, 'users', user.uid);
      const userDoc = await getDoc(userDocRef);
      const updatedSessions = [...(userDoc.data().sessions || []), newSession];
  ...
  const handleAnxietyChange = (level) => {
    ...
    saveAnxietyLevel(level);
  };

  const handleActivityComplete = async () => {
    await saveSession(selectedActivity);
    setCurrentPage('home');
        ...
        <div className="w-full max-w-md bg-slate-800 rounded-3xl p-8 shadow-xl">
          ...
          <div className="mt-6 p-4 bg-slate-700/50 rounded-xl border border-slate-600">
            <p className="text-xs text-slate-400 text-center">
              For testing, use any email and password. Your data will be saved to your account.
            </p>
          </div>
        </div>
      </div>
            ...
            {recentSessions.length > 0 ? recentSessions.map(session => (
              <div key={session.id} className="bg-slate-50 dark:bg-slate-800 rounded-2xl p-4 flex justify-between items-center">
                ...
                <div className="text-right text-sm text-slate-500">
                  {new Date(session.timestamp?.toDate?.() || session.timestamp).toLocaleDateString()}
                </div>
              </div>
            )) : (
